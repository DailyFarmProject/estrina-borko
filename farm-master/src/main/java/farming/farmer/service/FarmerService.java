...
//		new ResponseStatusException(HttpStatus.NOT_FOUND, "Farmer not found"));
        ...
        return farmerRepo.findById(farmerId).map(farmer -> {
...
//		return farmerRepo.findAll().stream().map(Farmer::build).collect(Collectors.toList());
    ...
    @Override
    @Transactional
    public void createFarmerProfile(User user) {
...
//        farmerRepo.save(farmer);
        log.info("Creating farmer profile for user: {}", user.getEmail());
        if (farmerRepo.findByUserAccountEmail(user.getEmail()).isPresent()) {
            log.warn("Farmer profile already exists for login: {}", user.getEmail());
            throw new ResponseStatusException(HttpStatus.CONFLICT, "Farmer profile already exists for login: " + user.getEmail());
        }
        Farmer farmer = Farmer.builder()
                //.userAccount(user)
                .phone(user.getPhone())
        ...
        log.info("Farmer profile created for login: {} with phone: {}, email: {}, address: {}",
                user.getEmail(), farmer.getPhone(), farmer.getEmail(),
