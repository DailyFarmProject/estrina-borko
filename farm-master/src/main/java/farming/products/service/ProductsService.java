...
public class ProductsService implements IProductsService {
    ...
    private void checkFarmerRole(User user) {
        if (user == null || !user.getUserType().equals(UserType.FARMER)) {
            log.warn("User {} is not a farmer, access denied", user != null ? user.getEmail() : "null");
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, "Only farmers can perform this operation");
        }
    }

    private void checkCustomerRole(User user) {
        if (user == null || !user.getUserType().equals(UserType.CUSTOMER)) {
            log.warn("User {} is not a customer, access denied", user != null ? user.getEmail() : "null");
    ...
    @Override
    @Transactional
    public boolean updateProduct(ProductDto productDto, User user) {
        log.info("Updating product with ID: {} by user: {}", productDto.getProductId(), user.getEmail());
        checkFarmerRole(user);
    ...
    @Override
    @Transactional
    public RemoveProductDataDto removeProduct(Long productId, Long farmerId, User user) {
        ...
        log.debug("Checking if user {} matches farmer ID {}", user.getEmail(), farmerId);
        if (!farmer.getUserAccount().getEmail().equals(user.getEmail())) {
            log.warn("User {} is not the owner farmer ID {}, access denied", user.getEmail(), farmerId);
    ...
    @Override
    @Transactional
    public SaleRecordsDto buyProduct(Long customerId, Long productId, int quantity, User user) {
        log.info("Customer ID {} buying {} units of product ID {} by user: {}", customerId, quantity, productId, user != null ? user.getEmail() : "null");
        if (user == null) {
            log.error("User is not authenticated");
            throw new ResponseStatusException(HttpStatus.UNAUTHORIZED, "User must be authenticated");
        }
        ...
        log.debug("Checking if user {} matches customer ID {}", user.getEmail(), customerId);
        if (!customer.getUserAccount().getEmail().equals(user.getEmail())) {
            log.warn("User {} is not the customer ID {}, access denied", user.getEmail(), customerId);
    ...
    @Override
    public List<ProductDto> getSoldProducts(Long farmerId, User user) {
        log.info("Fetching sold products for farmer ID: {} by user: {}", farmerId, user.getEmail());
        checkFarmerRole(user);
        Farmer farmer = farmerRepo.findById(farmerId)
                .orElseThrow(() -> {
                    log.error("Farmer not found with ID: {}", farmerId);
    ...
    @Override
    @Transactional
    public SaleRecordsDto buySurpriseBag(Long customerId, Long surpriseBagId, User user) {
